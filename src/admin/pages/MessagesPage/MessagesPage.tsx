...
const VARIABLES_OMW = [
  { tag: "{name}", desc: "Customer's first name" },
  { tag: "{address}", desc: "Service address" },
  { tag: "{eta}", desc: "ETA you pick before sending" },
];
...
const MessagesPage = () => {
  ...
  const fetchAll = useCallback(async () => {
    ...
    if (settRes.data) {
      ...
      setConfirmEnabled(m.sms_confirmation_enabled !== "false");
      setOmwEnabled(m.sms_omw_enabled !== "false");
    }
  ...
  const saveConfirmTemplate = async () => {
    setSavingConfirm(true);
    await supabase
      .from("admin_settings")
      .upsert({ key: "sms_confirmation_template", value: confirmTemplate, updated_at: new Date().toISOString() });
    toast({ title: "Confirmation message saved ✓" });
    setConfirmSaved(true);
    setSavingConfirm(false);
  };

  const saveOmwTemplate = async () => {
    setSavingOmw(true);
    await supabase
      .from("admin_settings")
      .upsert({ key: "sms_omw_template", value: omwTemplate, updated_at: new Date().toISOString() });
    toast({ title: "OMW message saved ✓" });
    setOmwSaved(true);
    setSavingOmw(false);
  };
  ...
  const saveSettings = async () => {
    ...
    await Promise.all([
      supabase.from("admin_settings").upsert({ key: "twilio_from_number", value: fromNumber, updated_at: new Date().toISOString() }),
      supabase.from("admin_settings").upsert({ key: "sms_confirmation_enabled", value: String(confirmEnabled), updated_at: new Date().toISOString() }),
      supabase.from("admin_settings").upsert({ key: "sms_omw_enabled", value: String(omwEnabled), updated_at: new Date().toISOString() }),
    ]);
  ...
  return (
    <div className="p-4 md:p-6 max-w-4xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        {/* Auto-confirmation */}
        <div className={`rounded-xl border bg-card p-4 flex items-center gap-3 ${!confirmEnabled ? "opacity-60" : ""}`}>
          <div className={`h-10 w-10 rounded-full flex items-center justify-center shrink-0 ${confirmEnabled ? "bg-primary/10" : "bg-muted"}`}>
            <Zap className={`h-5 w-5 ${confirmEnabled ? "text-primary" : "text-muted-foreground"}`} />
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-sm font-semibold">Auto Confirmation</p>
            <p className="text-xs text-muted-foreground">
              {confirmEnabled ? "Texts customer instantly on new booking" : "Currently disabled"}
            </p>
          </div>
          {confirmEnabled
        ...
        {/* Twilio status */}
        <div className={`rounded-xl border bg-card p-4 flex items-center gap-3 ${!twilioReady ? "border-amber-300 dark:border-amber-700" : ""}`}>
          <div className={`h-10 w-10 rounded-full flex items-center justify-center shrink-0 ${twilioReady ? "bg-green-100 dark:bg-green-900/30" : "bg-amber-100 dark:bg-amber-900/30"}`}>
            <Phone className={`h-5 w-5 ${twilioReady ? "text-green-600" : "text-amber-600"}`} />
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-sm font-semibold">Twilio SMS</p>
            <p className={`text-xs truncate ${twilioReady ? "text-muted-foreground" : "text-amber-600 dark:text-amber-400"}`}>
                  ...
                  <div className="flex items-start gap-4">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="grid grid-cols-2 gap-x-8 gap-y-1 text-xs text-muted-foreground">
                        <span className="flex items-center gap-1.5"><Clock className="h-3 w-3 shrink-0" />{b.booking_time || "Time TBD"}</span>
                        <span className="flex items-center gap-1.5"><Phone className="h-3 w-3 shrink-0" />{b.phone}</span>
                        <span className="flex items-center gap-1.5 truncate"><Wrench className="h-3 w-3 shrink-0" />{b.service || "—"} · {b.vehicle_size}</span>
                        <span className="flex items-center gap-1.5 truncate"><MapPin className="h-3 w-3 shrink-0" />{b.address}, {b.city}</span>
                      </div>
                    </div>

                    <Button
                      size="sm"
                      className={`shrink-0 gap-1.5 ${isExpanded ? "bg-blue-700 hover:bg-blue-700" : "bg-blue-600 hover:bg-blue-700"} text-white`}
                      disabled={!omwEnabled}
                    ...
                    <div className="bg-muted/40 rounded-xl border p-4 space-y-3">
                      {!twilioReady && (
                        <div className="flex items-center gap-2 text-xs text-amber-700 dark:text-amber-400 bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-700 rounded-lg p-2.5">
                          ...
                          <button className="underline font-medium" onClick={() => { setExpandedOmw(null); setSettingsOpen(true); }}>
                            set it up first
                          </button>
                        </div>
                      )}
                      <div>
                        <p className="text-xs font-medium mb-2 text-muted-foreground">How long until you arrive?</p>
                        <div className="flex flex-wrap gap-1.5 mb-2">
                          {ETA_PRESETS.map((e) => (
                            <button key={e}
                              onClick={() => setOmwEtas((prev) => ({ ...prev, [b.id]: e }))}
                              className={["rounded-full border px-3 py-1 text-xs font-medium transition-colors",
                                eta === e ? "border-primary bg-primary text-primary-foreground" : "border-border bg-background hover:border-primary/60",
                              ].join(" ")}>
                              {e}
                            </button>
                          ))}
                        </div>
                        <Input className="h-8 text-xs" placeholder="Or type a custom ETA…"
                          value={eta} onChange={(e) => setOmwEtas((prev) => ({ ...prev, [b.id]: e.target.value }))} />
                      </div>
                      <div className="rounded-lg bg-background border p-3 text-xs leading-relaxed text-muted-foreground whitespace-pre-wrap min-h-[48px]">
                        {buildOmwPreview(b, eta) || <span className="italic">Set up your OMW template below to preview the message</span>}
                      </div>
                      <div className="flex justify-end gap-2">
                        <Button size="sm" variant="outline" className="h-8 text-xs" onClick={() => setExpandedOmw(null)}>Cancel</Button>
                        <Button size="sm" className="h-8 text-xs gap-1.5 bg-blue-600 hover:bg-blue-700 text-white"
                          disabled={!twilioReady || sending === b.id} onClick={() => sendOmw(b)}>
                          {sending === b.id
      ...
      <div className="rounded-xl border bg-card">
        <div className="px-5 py-4 border-b">
          ...
          <p className="text-xs text-muted-foreground mt-0.5">
            Edit what gets sent to customers. Changes save immediately when you click Save.
          </p>
        </div>
          ...
          <div className="px-5 py-5 space-y-3">
            <div className="flex items-center justify-between gap-3">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="text-sm font-semibold">Booking Confirmation</p>
                  <p className="text-xs text-muted-foreground">Sent automatically when a new booking comes in</p>
                </div>
              </div>
              {!confirmSaved && (
            ...
            <Textarea
              ...
              onChange={(e) => { setConfirmTemplate(e.target.value); setConfirmSaved(false); }}
              rows={4}
              className="text-sm font-mono resize-none"
              placeholder={"Hey {name}! Your booking for {service} on {date} at {time} is confirmed. We'll see you at {address}! — Glossworks 🚗"}
            />
            ...
            <div className="flex items-center justify-between gap-3 flex-wrap">
              <div className="flex flex-wrap gap-1.5">
                {VARIABLES_CONFIRMATION.map((v) => (
                  <button key={v.tag}
                    onClick={() => { setConfirmTemplate((t) => t + v.tag); setConfirmSaved(false); }}
                    title={v.desc}
                    className="text-[11px] bg-muted hover:bg-primary/10 border rounded px-1.5 py-0.5 font-mono transition-colors">
                    {v.tag}
                  </button>
                ))}
              </div>
              <div className="flex items-center gap-3 shrink-0">
                <span className="text-[11px] text-muted-foreground">{confirmTemplate.length} chars</span>
                <Button size="sm" className="h-8 text-xs gap-1.5" disabled={confirmSaved || savingConfirm} onClick={saveConfirmTemplate}>
                  {savingConfirm ? <><RefreshCw className="h-3 w-3 animate-spin" /> Saving…</> : <><Save className="h-3 w-3" /> Save</>}
                </Button>
              </div>
            </div>
          </div>
          ...
          <div className="px-5 py-5 space-y-3">
            <div className="flex items-center justify-between gap-3">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="text-sm font-semibold">On My Way</p>
                  <p className="text-xs text-muted-foreground">Sent manually from Today's Schedule before each job</p>
                </div>
              </div>
              {!omwSaved && (
                <span className="text-[11px] text-amber-600 dark:text-amber-400 font-medium">Unsaved changes</span>
            ...
            <Textarea
              value={omwTemplate}
              onChange={(e) => { setOmwTemplate(e.target.value); setOmwSaved(false); }}
              rows={3}
              className="text-sm font-mono resize-none"
            ...
            <div className="flex items-center justify-between gap-3 flex-wrap">
              <div className="flex flex-wrap gap-1.5">
                {VARIABLES_OMW.map((v) => (
                  <button key={v.tag}
                    onClick={() => { setOmwTemplate((t) => t + v.tag); setOmwSaved(false); }}
                    title={v.desc}
                    className="text-[11px] bg-muted hover:bg-primary/10 border rounded px-1.5 py-0.5 font-mono transition-colors">
                    {v.tag}
                  </button>
              ...
              <div className="flex items-center gap-3 shrink-0">
                <span className="text-[11px] text-muted-foreground">{omwTemplate.length} chars</span>
                <Button size="sm" className="h-8 text-xs gap-1.5" disabled={omwSaved || savingOmw} onClick={saveOmwTemplate}>
                  {savingOmw ? <><RefreshCw className="h-3 w-3 animate-spin" /> Saving…</> : <><Save className="h-3 w-3" /> Save</>}
                </Button>
              </div>
            </div>
          </div>
              ...
              <tbody>
                {history.map((m) => (
                  <tr key={m.id} className="border-b last:border-b-0 hover:bg-muted/20 transition-colors">
                    <td className="px-5 py-2.5 text-xs text-muted-foreground whitespace-nowrap">{fmtTime(m.created_at)}</td>
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-6 py-1">
            ...
            <div className="space-y-2">
              <Label className="text-sm font-semibold">Twilio Phone Number</Label>
              <Input placeholder="+15025551234" value={fromNumber} onChange={(e) => setFromNumber(e.target.value)} />
              <p className="text-xs text-muted-foreground">The number texts are sent FROM. Format: +1XXXXXXXXXX</p>
            </div>
            ...
            <div className="space-y-4">
              <p className="text-sm font-semibold">Message Toggles</p>
              <p className="text-xs text-muted-foreground -mt-2">
                Turn these off instantly if something goes wrong. Templates are preserved.
              </p>
              ...
              <div className="flex items-center justify-between gap-4 p-3 rounded-lg border bg-muted/30">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">Auto-send on new booking</p>
                  </div>
                </div>
                <Switch
              ...
              <div className="flex items-center justify-between gap-4 p-3 rounded-lg border bg-muted/30">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium">On My Way</p>
                    <p className="text-xs text-muted-foreground">Manual send from Today's Schedule</p>
                  </div>
                </div>
                <Switch
                  checked={omwEnabled}
                  onCheckedChange={setOmwEnabled}
                />
              </div>
            </div>
            ...
            {/* Twilio setup guide (collapsed if already configured) */}
            {!twilioReady && (
              <div className="rounded-xl bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800 p-4 space-y-2">
                <p className="text-xs font-semibold text-amber-800 dark:text-amber-400">How to connect Twilio:</p>
                <ol className="text-xs text-amber-700 dark:text-amber-500 space-y-1.5 list-decimal list-inside">
                  <li>Sign up at <a href="https://twilio.com" target="_blank" rel="noopener noreferrer" className="underline">twilio.com</a> (free $15 trial)</li>
                  <li>Get a Twilio phone number (~$1/mo)</li>
                  <li>Go to Supabase → Project Settings → Edge Functions → Secrets</li>
                  <li>
                    Add these two secrets:
                    <code className="block mt-1 bg-amber-100 dark:bg-amber-900/40 rounded p-2 text-[11px] leading-relaxed">
                      TWILIO_ACCOUNT_SID = ACxxxxxxx{"\n"}
                      TWILIO_AUTH_TOKEN = your_token
                    </code>
                  </li>
                  <li>Enter your Twilio number above and save</li>
                </ol>
              </div>
            )}
          </div>
