...
const CalendarPage = () => {
  ...
  const getRanges = useCallback(() => {
    ...
    const mo = anchor.getMonth();
    return {
      from: ymd(new Date(y, mo, 1)),
      to: ymd(new Date(y, mo + 1, 0)),
    };
  }, [anchor, view]);
  ...
  const fetchData = useCallback(async () => {
    ...
    const [bRes, cbRes] = await Promise.all([
      supabase
  ...
  const manualBlocksOnDay = (d: string) =>
    blocks.filter((bl) => {
      const blDate = bl.start_at.slice(0, 10);
      return blDate === d && bl.block_type !== "booking";
    });

  const isDayFullyBlocked = (d: string) =>
    blocks.some(
      (bl) => bl.start_at.slice(0, 10) === d && bl.all_day && bl.block_type !== "booking"
    );
  ...
  const isHourBlocked = (d: string, h: number) =>
    blocks.some((bl) => {
      if (bl.block_type === "booking") return false;
      if (bl.start_at.slice(0, 10) !== d) return false;
  ...
  const handleBlockSave = async () => {
    ...
    setBlockSaving(true);
    const start = blockForm.allDay
      ? new Date(blockForm.date + "T00:00:00-04:00").toISOString()
      : new Date(blockForm.date + "T" + blockForm.startTime + ":00-04:00").toISOString();
    const end = blockForm.allDay
      ? new Date(blockForm.date + "T23:59:59-04:00").toISOString()
      : new Date(blockForm.date + "T" + blockForm.endTime + ":00-04:00").toISOString();

    const { error } = await supabase.from("calendar_blocks").insert({
      start_at: start,
      end_at: end,
      all_day: blockForm.allDay,
      note: blockForm.note || "Admin block",
      block_type: "manual",
    });
    if (error) toast({ title: "Error", description: error.message, variant: "destructive" });
      ...
      setBlockModal(false);
      setBlockForm({ allDay: true, date: todayStr(), startTime: "09:00", endTime: "17:00", note: "" });
      fetchData();
    }
  ...
  const handleRescheduleBooking = async () => {
    ...
    // Delete old calendar block tied to this booking and create new one
    await supabase
      .from("calendar_blocks")
      .delete()
      .eq("estimate_request_id", selectedBooking.id);
    ...
    const newEnd = new Date(
      new Date(`${rescheduleForm.date}T${padH}:00:00-04:00`).getTime() +
      (selectedBooking.duration_mins || 120) * 60000
    ).toISOString();
    ...
    await supabase.from("calendar_blocks").insert({
      ...
      estimate_request_id: selectedBooking.id,
      block_type: "booking",
    });
  ...
  const handleRainDay = async () => {
    ...
    // Create a reschedule_request record
    await supabase.from("reschedule_requests").insert({
      day: rainForm.fromDate,
      reason: rainForm.reason,
      status: "pending",
    });
    ...
    await supabase.from("calendar_blocks").insert({
      ...
      all_day: true,
      note: `🌧 Rain Day — ${rainForm.reason}`,
      block_type: "rain_day",
    });

    toast({
      title: "Rain day set ✓",
      description: `${fmtShortDate(rainForm.fromDate)} is blocked. Reschedule request logged.`,
    });
    setRainModal(false);
      ...
      <div>
        ...
        <div className="grid grid-cols-7 gap-px bg-border rounded-lg overflow-hidden border">
          {cells.map(({ date, cur }) => {
            ...
            const hasManualBlocks = manualBlocksOnDay(date).length > 0;
            ...
            return (
              <button
                ...
                className={[
                  "min-h-[80px] p-2 text-left transition-colors relative bg-background",
                  !cur ? "opacity-30" : "",
                  isPast && cur ? "opacity-60" : "",
              ...
              >
                ...
                <div className="mt-1 space-y-0.5">
                  {dayBookings.slice(0, 3).map((b) => (
                    ...
                    >
                      {b.booking_time ? b.booking_time.replace(/ (AM|PM)/, "$1") : ""} {b.name.split(" ")[0]}
                    </div>
                  ))}
            ...
            {days.map((d, i) => {
              ...
              return (
                ...
                >
                  ...
                  <div
                    className={[
                      "mx-auto mt-0.5 text-sm font-semibold w-7 h-7 rounded-full flex items-center justify-center",
          ...
          {HOURS.map((h) => (
            ...
            >
              ...
              {days.map((d) => {
                ...
                const isFirst = bForHour.length > 0 && parseHour24(bForHour[0].booking_time || "9:00 AM") === h;
                ...
                return (
                  ...
                  >
                    {bForHour.map((b) => {
                      const startH = parseHour24(b.booking_time || "9:00 AM");
                      if (startH !== h) return null; // only render at start hour
                      return (
                        <button
                          key={b.id}
                          onClick={() => setSelectedBooking(b)}
                          className={[
                            "w-full text-left rounded p-1.5 text-[10px] font-medium leading-tight mb-0.5",
                            b.completed
                              ? "bg-muted text-muted-foreground"
                    ...
                    {blocked && bForHour.length === 0 && (
                      <div className="text-[9px] text-red-400 p-1">Blocked</div>
                    )}
                  </div>
                );
              })}
  ...
  const DayPanel = () => {
    ...
    const isBlocked = isDayFullyBlocked(selectedDay);
            ...
            {!isPast && (
              <>
                <Button
                  ...
                  onClick={() => {
                    setBlockForm((f) => ({ ...f, date: selectedDay, allDay: true }));
                    setBlockModal(true);
                  }}
                >
                  <Ban className="h-3 w-3" /> Block Day
                </Button>
                <Button
                  size="sm"
                  variant="outline"
                  ...
                  onClick={() => {
                    setRainForm((f) => ({ ...f, fromDate: selectedDay }));
                    setRainModal(true);
                  }}
                >
        ...
        <div className="p-4">
          {dayBookings.length === 0 && dayManualBlocks.length === 0 ? (
            <p className="text-sm text-muted-foreground text-center py-4">
              No bookings or blocks for this day.
            </p>
          ) : (
            <div className="space-y-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs text-muted-foreground">
                        <span className="flex items-center gap-1">
                          ...
                          {b.booking_time} · {b.duration_mins} min
                        </span>
                        <span className="flex items-center gap-1">
                        ...
                        <>
                          <Button
                            ...
                            onClick={() => {
                              ...
                              setRescheduleForm({ date: b.booking_date, time: "" });
              ...
              {/* Manual blocks for the day */}
              {dayManualBlocks.map((bl) => (
                <div
                  key={bl.id}
                  className="rounded-lg border border-red-200 bg-red-50 dark:bg-red-950/20 p-3 flex items-center justify-between"
                >
                  <div>
                    <div className="flex items-center gap-2">
                      <Ban className="h-3.5 w-3.5 text-red-500" />
                      <span className="text-sm font-medium text-red-700 dark:text-red-300">
                        {bl.all_day ? "All Day Block" : `${new Date(bl.start_at).toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit" })} – ${new Date(bl.end_at).toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit" })}`}
                      </span>
                    </div>
                    {bl.note && (
                      <p className="text-xs text-red-600 dark:text-red-400 mt-0.5 ml-5">{bl.note}</p>
                    )}
                  </div>
                  <Button
                    size="icon"
    ...
    <div className="p-4 md:p-6 max-w-6xl mx-auto">
      ...
      <div className="flex items-center gap-4 mb-3 text-xs text-muted-foreground">
        <span className="flex items-center gap-1.5">
          <span className="w-3 h-3 rounded bg-primary/20" /> Booking
        </span>
        <span className="flex items-center gap-1.5">
          <span className="w-3 h-3 rounded bg-red-100 dark:bg-red-900/30" /> Blocked
        </span>
        <span className="flex items-center gap-1.5">
          <span className="w-3 h-3 rounded-full bg-primary" /> Today
        </span>
        <span className="flex items-center gap-1.5">
          <span className="w-3 h-3 rounded bg-muted" /> Completed
        </span>
      </div>
      ...
      <Dialog open={blockModal} onOpenChange={setBlockModal}>
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center gap-2">
              <Switch
                ...
                onCheckedChange={(v) => setBlockForm({ ...blockForm, allDay: v })}
              />
          ...
          <DialogFooter>
            ...
            <Button onClick={handleBlockSave} disabled={blockSaving || !blockForm.date}>
      ...
      <Dialog open={rescheduleModal} onOpenChange={setRescheduleModal}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle>Reschedule — {selectedBooking?.name}</DialogTitle>
          </DialogHeader>
          {selectedBooking && (
            <div className="space-y-3">
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {["8:00 AM","9:00 AM","10:00 AM","11:00 AM","12:00 PM","1:00 PM","2:00 PM","3:00 PM","4:00 PM"].map((t) => (
                    <button
                      key={t}
                      onClick={() => setRescheduleForm({ ...rescheduleForm, time: t })}
                      className={[
                        "rounded border px-2 py-1.5 text-xs transition-colors",
                        rescheduleForm.time === t
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => { setRescheduleModal(false); setSelectedBooking(null); }}>
              Cancel
            </Button>
            <Button
        ...
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <CloudRain className="h-4 w-4 text-blue-500" /> Rain Day
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-3">
            <p className="text-xs text-muted-foreground">
              Blocks the selected day and logs a reschedule request. Reach out to affected customers manually.
            </p>
            <div>
              <Label className="text-xs mb-1 block">Day to cancel</Label>
              <Input
                type="date"
                value={rainForm.fromDate}
                onChange={(e) => setRainForm({ ...rainForm, fromDate: e.target.value })}
              />
            </div>
            {rainForm.fromDate && bookingsOnDay(rainForm.fromDate).length > 0 && (
              <div className="rounded bg-orange-50 dark:bg-orange-950/20 border border-orange-200 p-2 text-xs text-orange-700 dark:text-orange-300">
                ⚠ {bookingsOnDay(rainForm.fromDate).length} booking{bookingsOnDay(rainForm.fromDate).length > 1 ? "s" : ""} on this day need rescheduling.
              </div>
            )}
            <div>
              <Label className="text-xs mb-1 block">Message / Reason</Label>
              <Textarea
                value={rainForm.reason}
                onChange={(e) => setRainForm({ ...rainForm, reason: e.target.value })}
                rows={2}
              />
            </div>
          </div>
          <DialogFooter>
            ...
            <Button
              onClick={handleRainDay}
              disabled={rainSaving || !rainForm.fromDate}
      ...
      {/* ── Booking Detail Modal (from week view click) ────────────────────── */}
      <Dialog
        open={!!selectedBooking && !rescheduleModal}
        onOpenChange={(o) => { if (!o) setSelectedBooking(null); }}
      >
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>{selectedBooking?.name}</DialogTitle>
          </DialogHeader>
          {selectedBooking && (
              ...
              {!selectedBooking.completed && (
                <div className="flex gap-2 pt-2">
                  <Button
                    ...
                    variant="outline"
                    onClick={() => {
                      setRescheduleForm({ date: selectedBooking.booking_date, time: "" });
                      setRescheduleModal(true);
                    }}
                  >
